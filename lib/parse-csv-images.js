    ...
    let csvStream = fastCsv({
        ...
        if (firstColumn) {
            for (let i = 0; i < data.length; i++) {
                switch (data[i]) {
                    ...
                    case 'external_id':
                        ...
                        break;
                    case 'image_width':
                        imageWidthColumnIndex = i;
                        break;
                    case 'image_height':
                        imageHeightColumnIndex = i;
                        break;
                }
            }
            ...
            images[externalId] = {
                ...
                width: data[imageWidthColumnIndex],
                height: data[imageHeightColumnIndex]
            };
            images[externalId]['otherAttributes'] = {};
            let otherAttributes = images[externalId]['otherAttributes'];
            for (let i = 0; i < data.length; i++) {
                if (i === urlColumnIndex || i === nameColumnIndex || i === externalIdColumnIndex || i === imageWidthColumnIndex || i === imageHeightColumnIndex) {
                    continue;
                }
